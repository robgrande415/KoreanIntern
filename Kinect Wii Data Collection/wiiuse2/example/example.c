...
#define wii_width 431.048
#define wii_depth 230.00
float tr_cal, tl_cal, br_cal, bl_cal;
 ...
 */
void handle_event(struct wiimote_t* wm, char buf[]) {
	printf("\n\n--- EVENT [id %i] ---\n", wm->unid);
	...
	if (wm->exp.type == EXP_NUNCHUK || wm->exp.type == EXP_MOTION_PLUS_NUNCHUK) {
        ...
        if (firsttime==1) {
            //calibrate wii board
            printf("Calibrating wii board: Please sit it down\n \n");
            sleep(3);
            printf("Calibrating wii board: Please wait...\n \n");
            sleep(3);
            struct wii_board_t* wb = (wii_board_t*)&wm->exp.wb;
            tl_cal = wb->tl /2; //subtracting off weight seems to do something awful.
            tr_cal = wb->tr /2;
            bl_cal = wb->bl /2;
            br_cal = wb->br /2;
            //tl_cal = 0;
        ...
        total = total - tr_cal - tl_cal - br_cal - bl_cal;
		float x = ( ( (wb->tr - tr_cal + wb->br - br_cal) / total)*2 - 1 ) * wii_width/2;
		float y = ( ( (wb->tl - tl_cal + wb->tr - tr_cal) / total)*2 - 1 ) * wii_depth/2;
        ...
        //write to file every 30 times
        //int numlines = 30;
        static char text[30][100];
        static int counter=0; //number of samples since last iteration.
        static int bigCounter=1; //total of files written
        printf("Dump %i Complete \n",bigCounter);
        ...
        //write to text
        snprintf(text[counter], 100, "%f, %f, %f, %ld", total, x, y,(start.tv_sec * 1000000 + start.tv_usec));
        
        //write to file every 30 sample
        if (counter==29) {
            FILE *ptr_file;
...
int main(int argc, char** argv) {
    ...
    //Create temp file to write on
    FILE *ptr_file;
    struct timeval start;
    gettimeofday(&start, NULL);
    int BUFSIZE = 4+3+4+4+5;
    char buf[BUFSIZE];
    snprintf(buf, BUFSIZE, "../../data/temp.txt");
    ptr_file =fopen(buf, "w");
    fclose(ptr_file);
