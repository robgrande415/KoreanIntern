...
void DrawDepthMap(const xn::DepthMetaData& dmd, const xn::SceneMetaData& smd, char buf_kin[], char buf_kin_filt[],char buf_depth[])
{
    ...
    //out file
    std::ofstream ptr_file(buf_depth, std::ios::app );
    bool printfile = FALSE;
    bool isperson = FALSE;
    bool timeprint = TRUE;
    if (depth_map_counter % 60 == 0) {
        printfile = TRUE;
    }
    depth_map_counter++;
...
//		printf("Initialized depth texture: width = %d, height = %d\n", texWidth, texHeight);
		...
		for (nY=0; nY<g_nYRes; nY++)
		{
			for (nX=0; nX < g_nXRes; nX++, nIndex++)
			{
				...
				if (g_bDrawBackground || *pLabels != 0)
				{
                    ...
                    {
                        //this is the part of the script that records which pixels correspond to a human player
                        //it first prints the time stamp
                        if (timeprint && printfile) {
                            //time
                            struct timeval start;
                            gettimeofday(&start, NULL);
                            long curr_time =(start.tv_sec);
	...
	for (int i = 0; i < nUsers; ++i)
	{
		...
		if (g_bPrintID)
		{
			...
			glRasterPos2i(com.X, com.Y); //prints text
