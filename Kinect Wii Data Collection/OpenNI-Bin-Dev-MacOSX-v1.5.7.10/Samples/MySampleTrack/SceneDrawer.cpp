...
float filtJointPosition[100][WINDOW_SIZE] = { };
float rawJointPosition[100][WINDOW_SIZE] = { };
long timeVec[WINDOW_SIZE] = { };
...
float GetWeightedMean(float daArray[], float timeArray[], float timeOut, int iSize) {
    float dSum = 0;
    float weights[iSize];
    float weightSum = 0;
    ...
    //assign prop weight
    for (int i = 0; i< iSize; i++) {
        weights[i] = 1/(1+ (timeArray[i] - timeOut)*(timeArray[i] - timeOut)*25*25);
        weightSum += weights[i];
    }
    //normalize
    for (int i = 0; i< iSize; i++) {
...
//record joint position to file
void recordJointPos(XnUserID player, char buf_kin[], char buf_kin_filt[])
{
    //out file
    std::ofstream ptr_file (buf_kin, std::ios::app );
    std::ofstream ptr_file_filt (buf_kin_filt, std::ios::app );
    ...
    //v.push_back( XN_SKEL_WAIST); //Not available in data
    
    //v.push_back( XN_SKEL_LEFT_COLLAR);
    v.push_back( XN_SKEL_LEFT_SHOULDER);
    v.push_back( XN_SKEL_LEFT_ELBOW);
    //v.push_back( XN_SKEL_LEFT_WRIST);
    v.push_back( XN_SKEL_LEFT_HAND);
    v.push_back( XN_SKEL_LEFT_FINGERTIP);
    
    //v.push_back( XN_SKEL_RIGHT_COLLAR);
    v.push_back( XN_SKEL_RIGHT_SHOULDER);
    v.push_back( XN_SKEL_RIGHT_ELBOW);
    //v.push_back( XN_SKEL_RIGHT_WRIST);
    v.push_back( XN_SKEL_RIGHT_HAND);
    v.push_back( XN_SKEL_RIGHT_FINGERTIP);
    ...
    int counter = 0;
    for (XnSkeletonJoint it : v) {
        
        //prints 0's to file if joint is not observed
        g_UserGenerator.GetSkeletonCap().GetSkeletonJointPosition(player, it, joint);
        if (!g_UserGenerator.GetSkeletonCap().IsJointActive(it))
        {
            ptr_file << "0, 0, 0, ";
            ptr_file_filt << "0, 0, 0, ";
            continue;
        ...
        ptr_file << pt.X << ", " << pt.Y << ", " << pt.Z << ", ";
        
        //save to array for filtering
        for (int j =1; j<WINDOW_SIZE; j++) {
            rawJointPosition[counter][j-1] = rawJointPosition[counter][j];
            rawJointPosition[counter+1][j-1] = rawJointPosition[counter+1][j];
            rawJointPosition[counter+2][j-1] = rawJointPosition[counter+2][j];
        }
        //add one new point
        rawJointPosition[counter][WINDOW_SIZE-1] = pt.X;
        rawJointPosition[counter+1][WINDOW_SIZE-1] = pt.Y;
        rawJointPosition[counter+2][WINDOW_SIZE-1] = pt.Z;
        
        
        //filters
        //median filter
        filtJointPosition[counter][(WINDOW_SIZE-1)/2] =
                                GetMedian(rawJointPosition[counter], WINDOW_SIZE);
        filtJointPosition[counter+1][(WINDOW_SIZE-1)/2] =
                                GetMedian(rawJointPosition[counter+1], WINDOW_SIZE);
        filtJointPosition[counter+2][(WINDOW_SIZE-1)/2] =
                                GetMedian(rawJointPosition[counter+2], WINDOW_SIZE);
        
        //sliding window filter
        filtJointPosition[counter][(WINDOW_SIZE-1)/2] =
                            GetWeightedMean(filtJointPosition[counter], filtJointPosition[100-1], filtJointPosition[100-1][(WINDOW_SIZE-1)/2], WINDOW_SIZE);
        filtJointPosition[counter+1][(WINDOW_SIZE-1)/2] =
                            GetWeightedMean(filtJointPosition[counter+1], filtJointPosition[100-1], filtJointPosition[100-1][(WINDOW_SIZE-1)/2], WINDOW_SIZE);
        filtJointPosition[counter+2][(WINDOW_SIZE-1)/2] =
                            GetWeightedMean(filtJointPosition[counter+2], filtJointPosition[100-1], filtJointPosition[100-1][(WINDOW_SIZE-1)/2], WINDOW_SIZE);
        
        //print to file
        ptr_file_filt << filtJointPosition[counter][(WINDOW_SIZE-1)/2] << ", "
                        << filtJointPosition[counter+1][(WINDOW_SIZE-1)/2] << ", "
                        << filtJointPosition[counter+2][(WINDOW_SIZE-1)/2] << ", ";
    ...
    long curr_time =(start.tv_sec*1000000 + start.tv_usec);
    ptr_file << curr_time << std::endl;
    
    //add time to vect
    //moves everything back one
    for (int j =1; j<WINDOW_SIZE; j++) {
        timeVec[j-1] = timeVec[j];
    }
    timeVec[WINDOW_SIZE-1] = curr_time;
    
    ptr_file_filt << timeVec[(WINDOW_SIZE-1)/2] << std::endl;
...
void DrawDepthMap(const xn::DepthMetaData& dmd, const xn::SceneMetaData& smd, char buf_kin[], char buf_kin_filt[])
{
...
//		printf("Initialized depth texture: width = %d, height = %d\n", texWidth, texHeight);
	...
	for (int i = 0; i < nUsers; ++i)
	{
		...
		if (g_bDrawSkeleton && g_UserGenerator.GetSkeletonCap().IsTracking(aUsers[i]))
		{
            ...
            //record position
            recordJointPos(aUsers[i], buf_kin, buf_kin_filt);
